...
namespace AHRS
{
    ...
    public class MadgwickAHRS
    {
        ...
        public void Update_2( float gx, float gy, float gz, float ax, float ay, float az )
        {
		...
		// Normalise accelerometer measurement
		recipNorm = 1/(float)Math.Sqrt(ax * ax + ay * ay + az * az);
		ax *= recipNorm;
		ay *= recipNorm;
		az *= recipNorm;   
		...
		recipNorm = 1/(float)Math.Sqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalise step magnitude
		s0 *= recipNorm;
		s1 *= recipNorm;
		s2 *= recipNorm;
		s3 *= recipNorm;
	...
	// Integrate rate of change of quaternion to yield quaternion
        ...
        q3 += qDot4 * SamplePeriod;
	       ...
	       recipNorm = 1f / (float)Math.Sqrt(q1 * q1 + q2 * q2 + q3 * q3 + q0 * q0);    // normalise quaternion
