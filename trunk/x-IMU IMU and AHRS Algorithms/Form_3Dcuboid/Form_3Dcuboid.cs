...
namespace WindowsFormsApplication1
{
    ...
    public partial class Form_3Dcuboid : Form
    {
        ...
        //public variables
        //
        public string label_showA {get; set;}
        public string label_showB { get; set; }
        ...
        private void formUpdateTimer_Tick(object sender, EventArgs e)
        {
            lock (this)
            {
                ...
                //addc code
                //
                DrawLabel.Text = label_showA;
                string temp = Convert.ToString ( 1.23 );
                DatAx.Text = Convert.ToString (Accx);
                DatAy.Text = Convert.ToString ( Accy );
                DatAz.Text = Convert.ToString ( Accz );
                DatGx.Text = Convert.ToString ( Gyrox);
                DatGy.Text = Convert.ToString ( Gyroy );
                DatGz.Text = Convert.ToString (Gyroz );
                DatX.Text = Convert.ToString(X);// (X / (float)Math.PI * 180);
                DatY.Text = Convert.ToString(Y);//( Y/(float)Math.PI * 180 );
                DatZ.Text = Convert.ToString(Z);//(Z/(float)Math.PI *180);
                this.Text  = "3D" + label_showA;
